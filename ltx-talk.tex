...
\begin{syntax}
  ...
  \cs{title}[short-title=\meta{short title}]\marg{title} \\
  \cs{subtitle}\oarg{short subtitle}\marg{subtitle} \\
  \cs{subtitle}[short-subtitle=\meta{short subtitle}]\marg{subtitle} \\
  \cs{author}\oarg{short author}\marg{author}\\
  \cs{author}[short-author=\meta{short author}]\marg{author} \\
  \cs{institute}\oarg{short institute}\marg{institute} \\
  \cs{institute}[short-institute=\meta{short institute}]\marg{institute} \\
  \cs{date}\oarg{short date}\marg{date}\\
  \cs{date}[short-date=\meta{short date}]\marg{date} \\
\end{syntax}
\cls{ltx-talk} supports \cs{title}, \cs{subtitle}, \cs{author}, \cs{institute} and \cs{date}.
If only one \meta{option} is given, this will act as short version of the macro for the footer.
If no short version is provided, the long version will be used instead.
If multiple \meta{options} are given, they are interpreted as key-values.
However currently only one key-value for the short version is known.
